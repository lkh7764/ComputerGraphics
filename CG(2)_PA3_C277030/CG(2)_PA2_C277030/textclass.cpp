	...
	int screenWidth, int screenHeight, XMMATRIX baseViewMatrix)
{
	...
	result = m_Font->Initialize(device, FONT_DATA, FONT_DDS);
	if (!result)
	{
	...
	// Initialize the first sentence.
	result = InitializeSentence(&m_sentence2, 50, device);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetSetense1(const char* text, int positionX, int positionY,
	float red, float green, float blue, ID3D11DeviceContext* deviceContext)
{
	bool result;
	result = UpdateSentence(m_sentence1, text, positionX, positionY, red, green, blue, deviceContext);
	
	return result;
}
bool TextClass::SetSetense2(const char* text, int positionX, int positionY,
	float red, float green, float blue, ID3D11DeviceContext* deviceContext)
{
	bool result;
	result = UpdateSentence(m_sentence2, text, positionX, positionY, red, green, blue, deviceContext);
