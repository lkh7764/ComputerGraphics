...
bool ColorShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	brightness = ColorBrightness::FULL;
...
HRESULT ColorShaderClass::CompilePixelShader(LPCWSTR psFilename, ID3D10Blob*& psBuf, ID3D10Blob*& errmsg)
{
	switch (brightness)
	{
	case ColorBrightness::FULL:
		return D3DCompileFromFile(psFilename, NULL, NULL, "ColorPixelShader", "ps_5_0",
			D3D10_SHADER_ENABLE_STRICTNESS, 0, &psBuf, &errmsg);

	case ColorBrightness::HALF:
		return D3DCompileFromFile(psFilename, NULL, NULL, "HalfColorPixelShader", "ps_5_0",
			D3D10_SHADER_ENABLE_STRICTNESS, 0, &psBuf, &errmsg);
	}
}


void ColorShaderClass::SetColorBrightness(ColorBrightness b, ID3D11Device* device, HWND hwnd)
{
	brightness = b;

	InitializeShader(device, hwnd, L"./data/color.vs", L"./data/color.ps");
}
	...
	LPCWSTR vsFilename, LPCWSTR psFilename)
{
	...
	result = D3DCompileFromFile(
		...
		"vs_5_0",						// 쉐이더 버전
		D3D10_SHADER_ENABLE_STRICTNESS,	// 얼마나 정확하게
		0, 
		&vertexShaderBuffer, 
		&errorMessage
	);
	...
	if(FAILED(result))
	{
		...
		if(errorMessage)
		{
			// 컴파일 오류를 출력, vs는 c++오류만 알려주기 때문에 따로 오류 확인 과정이 필요함
			// 여기서 오류가 발생, 메시지 박스가 떴다면 vs code가 아닌 shader code에서 오류가 난 것이다
			OutputShaderErrorMessage(errorMessage, hwnd, (LPCSTR)vsFilename);
		}
		// If there was  nothing in the error message then it simply could not find the shader file itself.
		else MessageBox(hwnd, (LPCWSTR)vsFilename, (LPCWSTR)"Missing Shader File", MB_OK);

		return false;
	}

    // Compile the pixel shader code.
	result = CompilePixelShader(psFilename, pixelShaderBuffer, errorMessage);
	if(FAILED(result))
	{
		// If the shader failed to compile it should have writen something to the error message.
		if(errorMessage) OutputShaderErrorMessage(errorMessage, hwnd, (LPCSTR)psFilename);
		// If there was nothing in the error message then it simply could not find the file itself.
		else MessageBox(hwnd, (LPCWSTR)psFilename, (LPCWSTR)"Missing Shader File", MB_OK);
	...
	polygonLayout[0].SemanticName = "POSITION";					// 데이터 이름
	polygonLayout[0].SemanticIndex = 0;
	polygonLayout[0].Format = DXGI_FORMAT_R32G32B32_FLOAT;		// 데이터 타입
	polygonLayout[0].InputSlot = 0;
	polygonLayout[0].AlignedByteOffset = 0;
	polygonLayout[0].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
	polygonLayout[0].InstanceDataStepRate = 0;

	polygonLayout[1].SemanticName = "COLOR";					// 데이터 이름
	polygonLayout[1].SemanticIndex = 0;
	polygonLayout[1].Format = DXGI_FORMAT_R32G32B32A32_FLOAT;	// 데이터 타입
...
void ColorShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, LPCSTR shaderFilename)
{
	...
	MessageBox(hwnd, (LPCWSTR)"Error compiling shader.  Check shader-error.txt for message.", 
		(LPCWSTR)shaderFilename, MB_OK);
...
bool ColorShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, 
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix)
{
	// 메인 기능: 여태 집어넣은 렌더링 정보를 GPU가 알아먹을 수 있는 형태로 설정해주기
	// 아까 설정해준 매트릭스 정보를 다 가져옴
	// 그걸 또 뒤집어줌
