...
bool TextureShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, PATH_VS, PATH_PS);
	if (!result)
	{
		return false;
	}
...
bool TextureShaderClass::Render(ID3D11DeviceContext* deviceContext, int vertexCount, int instanceCount, 
...
void TextureShaderClass::ChangeFilterMode(D3D11_FILTER fmode, ID3D11Device* device, HWND hwnd)
{
	m_filterMode = fmode;

	InitializeShader(device, hwnd, PATH_VS, PATH_PS);
}
